...
Let's consider constructing a covariance matrix for temporally-correlated random effects using a user-defined function. One could do this directly using for loops in BUGS code, but it will be more computationally efficient to do it in a user-defined function.
...
expcov <- nimbleFunction(     
   run = function(times = double(1), rho = double(0), tau = double(0)) {
      returnType(double(2))
      n <- length(times)
      declare(result, double(2, c(n, n)))
      setSize(result, c(n, n))
      for(i in 1:n)
            for(j in 1:n)
...
Let's generate some time series data.
...
inits <- list(mu = 0, rho = .2, tau = 1, sigma = 1)
...
```{r, plot-fit, fig.height=4, fig.width=6}
postBurn <- 5001:niter
